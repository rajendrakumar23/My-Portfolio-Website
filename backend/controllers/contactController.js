...
setTimeout(() => {
  ...
  if (pass.includes(" ")) {
    console.error(
      "❌ EMAIL_PASS contains spaces — remove all spaces from the App Password in .env",
    );
  ...
  if (pass.length !== 16) {
    console.warn(
      `⚠️  EMAIL_PASS is ${pass.length} chars — Gmail App Passwords are exactly 16 chars`,
    );
  }
    ...
    t.verify((err) => {
      if (err) {
        ...
        if (
          ...
          console.error(
            "   → Root cause: Wrong App Password or 2-Step Verification not enabled.",
          );
          console.error(
            "   → Fix: Go to https://myaccount.google.com/apppasswords",
          );
          console.error(
            "   → Create a new App Password, copy it WITHOUT spaces into EMAIL_PASS",
          );
        }
      } else {
...
exports.sendMessage = async (req, res, next) => {
  try {
    ...
    `,
      ...
      <div style="font-family:Arial,sans-serif;">
        <h2>Hi ${name},</h2>
        <p>Thank you for contacting me. I received your message and will reply soon.</p>
      </div>
    `,
      }),
    ]);
